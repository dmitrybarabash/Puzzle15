...
class BestScoresPresenterTests
{
    ...
    public void LoadAndShowBestScores()
    {
        ...
        for (int i = 0; i < model.BestScores.Count; i++)
        {
            ...
            var labelNames = presenter.View.Labels.Find($"nameLabel{i + 1}", searchAllChildren: false);
            var labelMoves = presenter.View.Labels.Find($"movesLabel{i + 1}", searchAllChildren: false);
            var labelTimers = presenter.View.Labels.Find($"timerLabel{i + 1}", searchAllChildren: false);
